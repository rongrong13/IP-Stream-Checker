...
*   **Web 可视化面板**: `http://设备IP:8000/ipcheck` — 实时显示检测进度、日志,支持停止任务
*   **智能缓存**: 基于内容 MD5 的去重缓存,默认 10 分钟有效期;任务复用,多用户共享同一检测任务
*   **数据源可选**: `Ping0` / `ippure`,可选降级策略
*   **流媒体解锁检测**: 可配置检测哪些服务(默认核心 6 个),结果实时标注在节点名
...
4. **高级设置**: 可覆盖队列大小、缓存时长、数据源等(需在 `config.yaml` 打开 `SHOW_ADVANCED_SETTINGS`)
